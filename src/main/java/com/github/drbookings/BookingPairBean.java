...
public class BookingPairBean {
	...
	BookingPairBean(final BookingEntry2 checkIn, final BookingEntry2 stay, final BookingEntry2 checkOut) {
	...
	public BookingPairBean(final LocalDate date) {
	...
	public Set<BookingOrigin> bookingOriginsView() {
		...
		return toList().stream().map(e -> e.getBookingOrigin()).collect(Collectors.toCollection(LinkedHashSet::new));
	}
	...
	public boolean equals(final Object obj) {
		...
		if(obj == null)
			return false;
		if(obj instanceof BookingPairBean)
			return Objects.equals(toList(), ((BookingPairBean)obj).toList());
		return false;
	}
	...
	LocalDate getDate(final BookingEntry2... be) {
		...
		for(final BookingEntry2 e : be)
			if(date == null) {
				...
				throw new IllegalArgumentException("Dates do not match");
		// if be array empty, date is null
		return null;
	}
	...
	public Set<Guest> guestView() {
		...
		return toList().stream().map(e -> e.getGuest()).collect(Collectors.toCollection(LinkedHashSet::new));
	}
	...
	public boolean hasGuest(final Guest guest) {
		...
		return toList().stream().map(b -> b.getGuest()).filter(g -> g.equals(guest)).findFirst().isPresent();
	}
	...
	public boolean hasRoom(final Room room) {
		...
		return toList().stream().map(b -> b.getRoom()).filter(g -> g.equals(room)).findFirst().isPresent();
	}
